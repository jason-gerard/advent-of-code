...
row_expanded = points |> Enum.flat_map(fn row -> if Enum.all?(row, &(&1 == ".")), do: [row, row], else: [row] end)

expanded_points = row_expanded
|> Enum.zip_with(&Function.identity/1)
|> Enum.flat_map(fn row -> if Enum.all?(row, &(&1 == ".")), do: [row, row], else: [row] end)
|> Enum.zip_with(&Function.identity/1)
IO.inspect(expanded_points, charlists: :as_lists)

galaxies = expanded_points
|> Enum.with_index
...
distances = pairs
|> Enum.map(fn {{i1, j1}, {i2, j2}} -> abs(i1 - i2) + abs(j1 - j2) end)
|> Enum.sum
