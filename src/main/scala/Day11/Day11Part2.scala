...
object Day11Part2 extends App {
    ...
    @tailrec
    def step(grid: List[List[Coord]], currentStep: Int, numFlashes: Int): (List[List[Coord]], Int) = {
        if (grid.flatten.count(coord => coord.value == 0) == grid.flatten.size) println(s"Flashed is synchronized on step ${currentStep}")
        ...
        step(resetGrid, currentStep+1, numFlashes+flashedGrid._2)
    }
    ...
    val grid = step(gridWithIndex, 0, 0)
    printGrid(grid._1)
